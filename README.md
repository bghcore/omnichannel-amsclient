...
This package supports React Native via Metro bundler. The package uses dual exports (ESM/CJS) - Metro will automatically select the appropriate format based on your project configuration.
...
For NPM registry publishing (recommended):

1. Build with Vite: `npm run build`
1. Test: `npm test`
1. Pack: `npm pack .`
1. Publish: `npm publish` or install locally via `npm install [package].tgz`

This creates dual-package output (ESM + CJS) for modern package consumers.

### Build CDN Package

For blob storage deployment (legacy):

1. Clean previous builds: `npm run clean`
1. Compile with TypeScript: `npm run build:tsc`
1. Bundle for CDN: `BASE_URL=https://[blob] SDK_VERSION=[version] node .\esbuild.config.js`
...
**Note:** The esbuild CDN build requires `lib/index.js` which is generated by `build:tsc`, not the default `build` command.
